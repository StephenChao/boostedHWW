...
path_tree ="/data/pubfs/zhaoyz/Tree/V7/2017/Splitted/MC/" # Your dir for Ntuple file waiting for TransferTree,waiting for changing.
file_dir1='/data/pubfs/zhaoyz/Ntuple/V3/2017/Merged/MC/'           # Your dir for Tree file after TransferTree, waiting for changing.
        ...
        if process in das:
            ...
            for file in os.listdir(file_dir1+das):
                if file.endswith('.root'):
                    print("Should print:python runEDBR2PKUTree.py --inputfile \"%s/%s\" --outputfile \"%s/Tree_%s\" --year 2017 --sampleXS %s --Nevents %f --channel \"HWW\" --IsData 100"%(file_dir1+das,file,path_tree+das,file,process_dict[process],count_process))
                    os.system("python runEDBR2PKUTree.py --inputfile \"%s/%s\" --outputfile \"%s/Tree_%s\" --year \"2017\" --sampleXS %s --Nevents %ld --channel \"HWW\" --IsData 100"%(file_dir1+das,file,path_tree+das,file,process_dict[process],count_process))
