...
import ROOT
TreeMERGED = "/data/pubfs/zhaoyz/Tree/V5/Splitted/2017/Signal/"
...
for Process in ProcessDict :
    for Samples in os.listdir(NtupleMERGED) :
        if Process in Samples :
            ...
            for Files in os.listdir(NtupleMERGED + Samples):
                if Files.endswith('.root'):
                    print("Should print:python runEDBR2PKUTree.py --inputfile \"%s/%s\" --outputfile \"%s/Tree_%s\" --year 2017 --sampleXS %s --Nevents %f --channel \"HWW\" --IsData 100 -S"%(NtupleMERGED + Samples, Files, TreeMERGED + Samples,Files,ProcessDict[Process],nEvents_all))
                    os.system("python runEDBR2PKUTree.py --inputfile \"%s/%s\" --outputfile \"%s/Tree_%s\" --year \"2017\" --sampleXS %s --Nevents %ld --channel \"HWW\" --IsData 100 -S"%(NtupleMERGED + Samples, Files, TreeMERGED + Samples, Files,ProcessDict[Process],nEvents_all))
