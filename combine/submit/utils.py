...
def setup(args):
    if args.site == "cmsconnect":
        t2_local_prefix = "/ceph/cms/" #not used
        t2_prefix = "root://redirector.t2.ucsd.edu:1095" #not used
        proxy = "/ospool/cms-user/yuzhe/x509up_u12976"
    elif args.site == "lpc":
...
def parse_common_args(parser):
    ...
    parser.add_argument("--cards-tag", default="Apr4", help="cards dir tag", type=str)
    parser.add_argument(
        "--site",
        default="cmsconnect",
        help="computing cluster we're running this on",
        type=str,
        choices=["lpc", "ucsd","cmsconnect"],
    )
    ...
    run_utils.add_bool_arg(parser, "resonant", default=False, help="resonant or nonresonant")
