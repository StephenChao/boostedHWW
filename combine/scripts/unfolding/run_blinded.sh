...
options=$(getopt -o "wblsdrgtiu" --long "workspace,bfit,limits,significance,dfit,dfitasimov,resonant,gofdata,goftoys,impactsi,unfolding,impactsf:,impactsc:,bias:,seed:,numtoys:,mintol:" -- "$@")
eval set -- "$options"
...
if [ $resonant = 0 ]; then #doing nonresonant fits
    ...
    unblindedparams="--freezeParameters var{.*_In},var{.*__norm},var{n_exp_.*},r_WH_hww,r_ZH_hww,r_ttH_hww --setParameters $maskblindedargs"
...
if [ $workspace = 1 ]; then
    ...
    text2workspace.py $ws.txt -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose --PO 'map=.*/ggH_hww_200_300:r_ggH_pt200_300[1,-40,40]' --PO 'map=.*/ggH_hww_300_450:r_ggH_pt300_450[1,-40,40]' --PO 'map=.*/ggH_hww_450_Inf:r_ggH_pt450_inf[1,-40,40]' --PO 'map=.*/qqH_hww:r_qqH_hww[1,-40,40]' --PO 'map=.*/WH_hww:r_WH_hww[1,-40,40]' --PO 'map=.*/ZH_hww:r_ZH_hww[1,-40,40]' --PO 'map=.*/ttH_hww:r_ttH_hww[1,-40,40]' --channel-masks -o $wsm.root 2>&1 | tee $outsdir/text2workspace.txt
else
    if [ ! -f "$wsm.root" ]; then
