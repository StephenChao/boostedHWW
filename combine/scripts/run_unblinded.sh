...
if [ $impactsi = 1 ]; then
    ...
    --expectSignal 1 --rMin -20 --rMax 20 \
    -d $wsm.root --doInitialFit --robustFit 1 \
    --cminDefaultMinimizerTolerance $mintol \
    --cminDefaultMinimizerStrategy 0  2>&1 | tee $outsdir/Impacts_init.txt
fi
...
if [ "$impactsf" != 0 ]; then
    ...
    --cminDefaultMinimizerTolerance $mintol --X-rtd MINIMIZER_MaxCalls=400000 \
    --bypassFrequentistFit --toysFrequentist --expectSignal 1 --robustFit 1 \
    --setParameterRanges r=-10,20 --cminDefaultMinimizerStrategy 1 -v 1 -m 125 | tee "$outsdir/Impacts_$impactsf.txt"
